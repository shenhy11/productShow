...
- [x] 3.2 Implement `pages/products/index.vue` to fetch the category structure via `useAsyncData` + `mock/categories.json`.
- [x] 3.3 Create a dynamic Sidebar / Top-tab filter in `products/index.vue` to filter displayed `ZProductCard`s based on category.

## 4. Support Services & Advanced Interactions

- [x] 4.1 Develop `ZCascadeSelector.vue` taking nested tree data for the 3-tier inputs.
- [x] 4.2 Implement `pages/support/download.vue` with `lodash-es` debounce calling `useHttp` API (or mock JSON) when third tier (Model) is selected.
- [x] 4.3 Update `pages/support/email-support.vue` with strict Javascript form validations ensuring no empty subject, email, or message.
