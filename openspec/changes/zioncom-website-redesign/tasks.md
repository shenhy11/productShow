...
- [ ] 4.1 实现产品分类管理 API（树形 CRUD：新增/编辑/删除/排序/获取分类树）
- [ ] 4.2 实现产品 CRUD API（新增/编辑/上下架/删除/列表查询/详情查询）
- [ ] 4.3 实现产品规格参数管理 API（按产品 ID 批量保存/查询 key-value 键值对）
- [ ] 4.4 实现产品搜索 API（按关键词搜索型号/名称，按分类筛选，分页排序）
- [ ] 4.5 实现产品 API 多语言支持（?lang=en 参数返回对应语言字段）

## 5. 后端 - 内容管理模块 (zioncom-content)

- [ ] 5.1 实现文章 CRUD API（新增/编辑/发布/撤回/删除，按类型筛选 News/Event）
- [ ] 5.2 实现文章列表 API（分页、按类型筛选、按发布日期排序）
- [ ] 5.3 实现文章详情 API（含 SEO 元信息返回）
- [ ] 5.4 实现文章多语言支持

## 6. 后端 - 固件与技术支持模块 (zioncom-support)

- [ ] 6.1 实现固件版本管理 API（按产品上传固件、版本列表、Release Note）
- [ ] 6.2 实现用户手册/Datasheet 管理 API（关联产品上传 PDF）
- [ ] 6.3 实现 Download Center 搜索 API（按型号搜索固件和文档）
- [ ] 6.4 实现 FAQ 管理 CRUD API（问答对、分类、排序）
- [ ] 6.5 实现技术视频管理 API（标题、描述、视频嵌入 URL、关联产品）
- [ ] 6.6 实现联系表单/技术支持请求提交 API（保存到数据库）
- [ ] 6.7 实现 Web 模拟器管理 API（产品型号、模拟器链接）

## 7. 后端 - 品牌展示模块

- [ ] 7.1 实现里程碑管理 API（年份、事件标题、描述、CRUD）
- [ ] 7.2 实现全球渠道商管理 API（地区、名称、联系方式、经纬度、CRUD）
- [ ] 7.3 实现联系我们后台查看 API（联系记录列表、已读标记）

## 8. Admin 后台前端 (RuoYi-Vue3)

- [ ] 8.1 新增产品分类管理页面（树形表格、新增/编辑弹窗）
- [ ] 8.2 新增产品管理页面（列表表格 + 搜索筛选 + 新增/编辑表单含双语字段、封面图上传、富文本编辑器、规格参数动态表单、SEO 配置）
- [ ] 8.3 新增文章管理页面（列表 + 类型筛选 + 编辑表单含双语标题、富文本、封面图、SEO 配置）
- [ ] 8.4 新增固件管理页面（按产品筛选固件列表 + 上传固件表单含版本号和 Release Note）
- [ ] 8.5 新增文档管理页面（按产品管理手册和 Datasheet PDF）
- [ ] 8.6 新增 FAQ 管理页面（分类管理 + 问答对列表 + 编辑）
- [ ] 8.7 新增技术视频管理页面
- [ ] 8.8 新增里程碑管理页面（时间线数据编辑）
- [ ] 8.9 新增全球渠道商管理页面
- [ ] 8.10 新增联系记录 / 支持请求查看页面
- [ ] 8.11 新增文件管理页面（OSS 文件列表、筛选、删除）
- [ ] 8.12 配置 RuoYi 菜单和权限（为新增业务模块添加菜单项和按钮权限）
...
- [ ] 10.1 实现 Hero 区域（全屏大图/轮播，展示主推产品）
- [ ] 10.2 实现快速通道区域（产品搜索入口、OEM/ODM 能力介绍入口）
- [ ] 10.3 实现最新动态区域（最新新闻和事件卡片）
- [ ] 10.4 实现产品分类快速导航区域

## 11. Nuxt 前台 - 产品模块

- [ ] 11.1 实现产品列表页（/products）：分类侧边栏 + 产品卡片网格 + 分页
- [ ] 11.2 实现产品详情页（/products/:id）：产品大图、摘要、规格参数表、相关固件下载、相关文档下载
- [ ] 11.3 实现产品搜索功能（搜索框 + 搜索结果页）

## 12. Nuxt 前台 - 新闻与事件

- [ ] 12.1 实现新闻列表页（/news）：文章卡片列表 + 分页
- [ ] 12.2 实现事件列表页（/events）：事件卡片列表 + 分页
- [ ] 12.3 实现文章详情页（/news/:id、/events/:id）：标题、封面图、正文富文本、发布日期

## 13. Nuxt 前台 - 品牌展示

- [ ] 13.1 实现关于我们页面（/about）：企业简介、核心优势
- [ ] 13.2 实现里程碑页面（/about/milestones）：时间线组件，按年份展示
- [ ] 13.3 实现工厂与实验室页面（/about/factory-lab）：图片画廊 + 文字介绍
- [ ] 13.4 实现全球渠道页面（/worldwide）：交互式世界地图（可用 Leaflet/Mapbox）标注渠道商
- [ ] 13.5 实现联系我们页面（/about/contact）：联系信息 + 表单提交

## 14. Nuxt 前台 - 技术支持

- [ ] 14.1 实现 Download Center 页面（/support/download）：型号搜索 + 固件/文档结果列表 + 下载按钮
- [ ] 14.2 实现 FAQ 页面（/support/faq）：分类分组 + 折叠/展开问答
...
- [ ] 15.1 确定设计系统（配色方案、字体、间距、圆角等 Design Tokens）
- [ ] 15.2 实现前台全局 CSS 变量和基础样式
- [ ] 15.3 实现交互动效（页面过渡动画、Hover 效果、滚动触发动画）
- [ ] 15.4 适配暗色模式（可选，建议后期迭代）
